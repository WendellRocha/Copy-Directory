...
namespace Backup {
    public partial class Form1 : Form {
        ...
        private void salvarConfig(string caminho, string destinho) {
            Configuration config = ConfigurationManager.OpenExeConfiguration(ConfigurationUserLevel.None);
            config.AppSettings.Settings["caminho"].Value = caminho;
            config.AppSettings.Settings["destino"].Value = destino;
            config.Save(ConfigurationSaveMode.Modified);
        }

        private void carregarConfig() {
            var caminhoArquivo = Convert.ToString(ConfigurationManager.AppSettings["caminho"]);
            var destinoArquivo = Convert.ToString(ConfigurationManager.AppSettings["destino"]);
            caminhoDoArquivo.Text = caminhoArquivo;
            destinoDoArquivo.Text = destinoArquivo;
        }
        ...
        private void inverter_Click(object sender, EventArgs e) {
            string aux;
            aux = caminhoDoArquivo.Text.ToString();
            caminhoDoArquivo.Text = destinoDoArquivo.Text;
            destinoDoArquivo.Text = aux;
        }
    }
}
